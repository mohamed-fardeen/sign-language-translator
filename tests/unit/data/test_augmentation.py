...
def test_augment_clip_returns_correct_shapes() -> None:
    ...
    p, l, r, m = augment_clip(pose, lh, rh, mask)
    assert p.shape[0] == l.shape[0] == r.shape[0] == m.shape[0]
    assert p.shape[1] == 99 and l.shape[1] == 63 and r.shape[1] == 63
    assert m.dtype == bool
